...
class office(models.Model):
    number = models.CharField('Отдел', default="", max_length=9)
...
class Application(models.Model):
    ...
    number_cab = models.ForeignKey(office, on_delete=models.CASCADE, null=True, blank=True, verbose_name = 'Выберите отдел')
...
class Labs_cabinets(models.Model):
    cabinet = models.ForeignKey(office, on_delete=models.CASCADE, null=True, blank=True, verbose_name='Отдел работника')
    worker = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
    ...
    class Meta:
        ...
        verbose_name = 'Закрепленный за работником отдел'
        verbose_name_plural = 'Закрепеленные за работником отдел'
    def __str__(self):
        return f'Кабинет {self.cabinet} привязан к работнику {self.worker}'
